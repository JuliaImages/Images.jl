using Images, Lexicon

const api_directory = "api"
const modules = [Images]
...
cd(dirname(@__FILE__)) do
    ...
    index  = Index()
    for mod in modules
        update!(index, Lexicon.save(joinpath(api_directory, "$(mod).md"), mod))
    end
    Lexicon.save(joinpath(api_directory, "index.md"), index; md_subheader = :category)
