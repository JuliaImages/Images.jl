...
facts("show (MIME)") do
    ...
    @fact mimewritable(MIME("image/svg+xml"), rand(Gray{N0f8}, 5, 5)) --> false
    @fact mimewritable(MIME("image/svg+xml"), rand(RGB{N0f8},  5, 5)) --> false
    @fact mimewritable(MIME("image/png"), rand(Gray{N0f8}, 5, 5)) --> true
    @fact mimewritable(MIME("image/png"), rand(RGB{N0f8},  5, 5)) --> true
    ...
    context("no compression or expansion") do
        ...
        b = convert(Image{Gray{N0f8}}, load(fn))
        @fact data(b) --> A
    end
    ...
    context("big images (use of restrict)") do
        ...
        b = convert(Image{Gray{N0f8}}, load(fn))
        @fact data(b) --> convert(Array{N0f8}, Ar)
        ...
        b = convert(Image{Gray{N0f8}}, load(fn))
        abigui = convert(Array{N0f8,2}, data(restrict(abig, (1,2))))
        @fact data(b) --> abigui
    end
end
