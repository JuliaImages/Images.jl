@testset "show (MIME)" begin
    ...
    @test !(mimewritable(MIME("image/svg+xml"),rand(Gray{U8},5,5)))
    @test !(mimewritable(MIME("image/svg+xml"),rand(RGB{U8},5,5)))
    @test mimewritable(MIME("image/png"),rand(Gray{U8},5,5))
    @test mimewritable(MIME("image/png"),rand(RGB{U8},5,5))
    ...
    @testset "big images (use of restrict)" begin
        ...
        b = convert(Image{Gray{U8}}, load(fn))
        @test data(b) == convert(Array{U8},Ar)
