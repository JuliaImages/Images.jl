...
@testset "legacy" begin
    ...
    @testset "Restriction" begin
        ...
        A = reshape([convert(UInt16, i) for i in 1:60], 4, 5, 3)
        B = restrict(A, (1, 2))
    ...
    @testset "Features" begin
        ...
        A = zeros(Int, 9, 9, 9)
        A[5, 4:6, 5] .= 1
        blobs = blob_LoG(A, 2 .^ [1.0, 0, 0.5], [1.0, 3.0, 1.0])
        @test length(blobs) == 1
        @test blobs[1].location == CartesianIndex((5, 5, 5))
        A = zeros(Int, 9, 9, 9)
        A[1, 1, 4:6] .= 1
        blobs = filter(b -> b.amplitude > 0.1, blob_LoG(A, 2.0 .^ [0.5, 0, 1], true, [1.0, 1.0, 3.0]))
        @test length(blobs) == 1
        @test blobs[1].location == CartesianIndex((1, 1, 5))
        @test filter(b -> b.amplitude > 0.1, blob_LoG(A, 2.0 .^ [0.5, 0, 1], (true, true, true, false), [1.0, 1.0, 3.0])) == blobs
        @test isempty(blob_LoG(A, 2.0 .^ [0, 1], (false, true, false, false), [1.0, 1.0, 3.0]))
        A = zeros(Int, 9, 9)
        A[[1:2; 5], 5] .= 1
